(ns millcoord.actor
  "MillCoordActor — the ISCO-08 8152 weaving and knitting machine
  operators mill scheduling/logistics coordination actor as a
  `langgraph.graph/state-graph` (ADR-2607121000 / CLAUDE.md Actors
  section). One graph run = one mill-coordination operation request
  ...
  The unconditional invariant: the Mill Scheduling Coordination
  Advisor can never directly commit an operating record the
  MillCoordGovernor refuses, and can never make this actor finalize a
  machine-operation-execution decision (e.g. deciding to proceed with
  a specific weaving or knitting run) or a mill-safety-clearance
  decision (e.g. declaring a loom or knitting machine safe to operate,
  or a fabric batch safe for shipment), and can never override a mill
  ...
  regardless of disposition path. This actor coordinates MILL
  SCHEDULING/LOGISTICS ONLY — it never operates weaving/knitting
  equipment itself, and it never makes a mill-safety-clearance
  decision itself."
...
(defn build-graph
  "Build a compiled MillCoordActor graph. `store` implements
  `millcoord.store/Store`. `advisor` implements
  `millcoord.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:weaver-id (:weaver-id request)
                                    :op (:op proposal)
                                    :mill-id (:mill-id proposal)
                                    :payload proposal}]
